...
function getPendingMessage(condType, bumpConfig, conditionStatus) {
        ...
        if (customTemplate) {
            if (combineLogic === 'all') {
                const messages = unmetConditions.map(c => {
                    // ✅ Pass full conditionStatus for 'both' types
                ...
                const wrappedConditions = messages.map(m => {
                    // Check if already contains HTML structure
                ...
                if (closestCondition) {
                    ...
                    if (conditionText.includes('<div class="multi-product-notification">')) {
                        return conditionText; // Return as-is
                    }
    ...
    // Check if this type should use list-based message (multiple products)
    ...
    if (typesWithMultipleProducts.includes(condType)) {
        ...
        if (hasMultipleProducts) {
            // Use getConditionMessage which will generate list-based message
            return getConditionMessage(condType, conditionStatus, bumpConfig, conditionStatus);
        }
    }
    
    // Use getConditionMessage for 'both' and 'price-both' types
