...
function getPlaceholderValue(placeholder, bumpConfig, conditionStatus) {
    ...
    switch(key) {
        ...
        case 'remainingProduct':
            // 🔥 CONTEXT-AWARE: Check if we're in list item context
...
function getProductNameFormatted(bumpConfig, conditionStatus) {
    ...
    // ✅ CHECK: Internal key first (injected by list context)
...
function getMultipleProductsMessage(combineLogic, bumpConfig, productDetails) {
    ...
    // ✅ FIXED: Pass proper context so {bumpName} gets replaced
    ...
    const listItems = productDetails.map((detail, index) => {
        ...
        // ✅ COMPLETE CONTEXT: Include ALL necessary data for placeholder replacement
        const itemContext = {
            productDetails: [detail],
            remaining: detail.remaining,
            remainingProduct: detail.remaining,
            _productName: productName  // Inject product name for this specific item
        };
        
        // Create a temp config with the specific product for this iteration
        const itemBumpConfig = {
            ...bumpConfig,
            conditions: {
                requiredProduct: { 
                    id: detail.id,
                    minQuantity: detail.requiredQty
                }
            }
        };
        
        let itemText = replacePlaceholdersInMessage(itemTemplate, itemBumpConfig, itemContext);
...
function getPendingMessage(condType, bumpConfig, conditionStatus) {
    ...
    if (!bumpConfig.conditions.multipleConditions) {
        console.error('❌ Invalid condition format. Order bumps MUST use multipleConditions array.');
        console.info('Example:\n{\n  multipleConditions: [\n    { type: "total", minTotalQuantity: 5 },\n    { type: "product", requiredProduct: { id: 1, minQuantity: 2 }}\n  ],\n  combineLogic: "all"\n}');
        return `⚠️ Invalid configuration for ${bumpConfig.productName}`;
    }
    ...
    console.warn(`⚠️ Unknown combineLogic: ${combineLogic}. Using fallback.`);
    return `Unlock ${bumpConfig.productName}`;
}
...
function getSuccessMessage(condType, bumpName, bumpConfig) {
    ...
    // ❌ ERROR: Must use multipleConditions
    console.error('❌ Invalid condition format. Please use multipleConditions.');
