...
// ========== UTILITY FUNCTIONS ==========
/**
 * Get price for a specific product (unit price × quantity)
 * @param {number} productId - The product ID to check
 * @returns {number} - Total price of the product (price × qty)
 */
...
function getPendingMessage(condType, bumpConfig, conditionStatus) {
    ...
    if (bumpConfig.conditions.multipleConditions) {
        ...
        // ✅ Check if single condition - use pendingSingle if available
        const isSingleCondition = unmetConditions.length === 1;
        
        const multipleConfig = defaultMessages['multiple']?.[combineLogic];
        let customTemplate = null;
        
        if (isSingleCondition && defaultMessages['multiple']?.pendingSingle) {
            // Use pendingSingle for single condition
            customTemplate = defaultMessages['multiple'].pendingSingle;
        } else if (typeof multipleConfig === 'string') {
        ...
        if (customTemplate) {
            // ✅ FIX: Pass original condition to getConditionMessage
            ...
            // ✅ Process template with badge placeholder
            let finalMessage = customTemplate
                .replace(/{conditions}/g, `<div class="conditions-text">${conditionsText}</div>`);
            
            finalMessage = replacePlaceholdersInMessage(finalMessage, bumpConfig, conditionStatus);

            return `<div class="multi-condition-wrapper">${finalMessage}</div>`;
        
        }
        
        return `Unlock ${generateBadgeHTML(bumpConfig)}`;
    }
 ...
 * @param {Object} bumpConfig - Bump configuration
 * @param {Object} originalCondition - Original condition object to identify correct combineLogic
 * @returns {string} - Formatted message
 */
function getConditionMessage(condType, details, bumpConfig, conditionStatus = null, originalCondition = null) {
    const defaultMessages = window.cartConfig.notifications?.messages || {};
    
    switch(condType) {
        case 'total': {
            // Get template from config or use fallback
            const template = defaultMessages['total']?.pending || 'Tambah {remainingTotal} item lagi untuk unlock {bumpName}';
            
            // ✅ FIX: Get actual values with proper fallback logic
            let totalQuantity = 1;
            let currentTotalQuantity = 0;
            
            // Priority: originalCondition → bumpConfig.conditions → default
            if (originalCondition && typeof originalCondition.minTotalQuantity !== 'undefined') {
                totalQuantity = originalCondition.minTotalQuantity;
            } else if (bumpConfig.conditions && typeof bumpConfig.conditions.minTotalQuantity !== 'undefined') {
                totalQuantity = bumpConfig.conditions.minTotalQuantity;
            }
            
            currentTotalQuantity = getTotalQuantity();
            
            const message = template
                .replace(/{totalQuantity}/g, `<strong>${totalQuantity}</strong>`)
                .replace(/{currentTotalQuantity}/g, `<strong>${currentTotalQuantity}</strong>`)
                .replace(/{remainingTotalQuantity}/g, `<strong>${details.remainingTotal}</strong>`)
                .replace(/{remainingTotal}/g, `<strong>${details.remainingTotal}</strong>`)
        ...
        case 'price-total': {
            // Get template from config or use fallback
            const template = defaultMessages['price-total']?.pending || 'Tambah lagi {remainingTotalPrice} untuk unlock {bumpName}';
            
            // ✅ FIX: Get actual values with proper fallback logic
            let totalPrice = 0;
            let currentTotalPrice = getTotalPrice();
            
            // Priority: originalCondition → bumpConfig.conditions → default
            if (originalCondition && typeof originalCondition.minTotalPrice !== 'undefined') {
                totalPrice = originalCondition.minTotalPrice;
            } else if (bumpConfig.conditions && typeof bumpConfig.conditions.minTotalPrice !== 'undefined') {
                totalPrice = bumpConfig.conditions.minTotalPrice;
            }
            
            const message = template
                .replace(/{totalPrice}/g, `<strong>${formatPrice(totalPrice)}</strong>`)
                .replace(/{currentTotalPrice}/g, `<strong>${formatPrice(currentTotalPrice)}</strong>`)
                .replace(/{remainingTotalPrice}/g, `<strong>${formatPrice(details.remainingTotalPrice)}</strong>`)
                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
        ...
        case 'product': {
            ...
            // Single product - NEW STANDARDIZED PLACEHOLDERS
            let productName = 'produk';
            let productDetail = details.productDetails?.[0] || {};
            ...
            const defaultMessages = window.cartConfig.notifications?.messages || {};
            const template = defaultMessages['product']?.pending || 'Tambah {remainingProductQuantity} {productName} lagi untuk unlock {bumpName}';
            ...
            const message = template
                ...
                .replace(/{remainingProductQuantity}/g, `<strong>${productDetail.remaining || details.remainingProduct}</strong>`)
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
        ...
        case 'price-product': {
            ...
            const message = template
                ...
                .replace(/{remainingProductPrice}/g, `<strong>${formatPrice(productDetail.remaining || details.remainingProductPrice)}</strong>`)
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
...
/**
 * Generate badge HTML with icon and text
 * @param {Object} bumpConfig - Bump configuration
 * @returns {string} - Badge HTML with strong tag
 */
function generateBadgeHTML(bumpConfig) {
    const icon = bumpConfig.badgeIcon || 'fa-gift';
    const text = bumpConfig.badgeText || 'TAWARAN ISTIMEWA';
    
    return `<strong class="notification-badge"><i class="fa ${icon}"></i> ${text}</strong>`;
}
...
function replacePlaceholdersInMessage(template, bumpConfig, conditionStatus) {
    ...
    // Badge placeholder (with icon + text)
    const badgeHTML = generateBadgeHTML(bumpConfig);
    message = message.replace(/{badge}/g, badgeHTML);
...
/**
 * Get success message with user-defined templates (ENHANCED VERSION)
 * Supports all condition types with full placeholder replacement
 * 
 * @param {string} condType - Condition type
 * @param {string} bumpName - Order bump name
 * @param {Object} bumpConfig - Full bump configuration
 * @param {Object} conditionStatus - OPTIONAL: Full condition status for detailed placeholders
 * @returns {string} - Formatted message
 */
function getSuccessMessage(condType, bumpName, bumpConfig, conditionStatus = null) {
    const defaultMessages = window.cartConfig.notifications?.messages || {};
    ...
    if (bumpConfig && bumpConfig.conditions && bumpConfig.conditions.multipleConditions) {
        ...
        // ✅ Check if single condition - use successSingle if available
        const totalConditions = bumpConfig.conditions.multipleConditions.length;
        const isSingleCondition = totalConditions === 1;
        
        let multipleSuccessMsg = null;
        
        // Priority: successSingle → combineLogic.success → fallback
        if (isSingleCondition && defaultMessages['multiple']?.successSingle) {
            multipleSuccessMsg = defaultMessages['multiple'].successSingle;
        } else {
        ...
        if (multipleSuccessMsg) {
            // ✅ Process with badge placeholder support
            return replacePlaceholdersInMessage(multipleSuccessMsg, bumpConfig, conditionStatus);
        }
        
        // Fallback to generic success message
        const fallbackMsg = defaultMessages['multiple']?.success || 'Tahniah! Anda layak untuk <strong>{bumpName}</strong> pada harga diskaun';
        
        return fallbackMsg.replace(/{bumpName}/g, `<strong>${bumpName}</strong>`);
    }
    
    // ========== SINGLE CONDITION WITH ENHANCED PLACEHOLDERS ==========
    ...
    // ✅ Process with badge placeholder support
    let message = replacePlaceholdersInMessage(template, bumpConfig, conditionStatus);
    
    // ========== ENHANCED: Add support for additional placeholders ==========
    if (conditionStatus && bumpConfig.conditions) {
        // For 'total' type
        if (condType === 'total') {
            const totalQuantity = bumpConfig.conditions.minTotalQuantity || 1;
        ...
        // For 'price-total' type
        if (condType === 'price-total') {
            const totalPrice = bumpConfig.conditions.minTotalPrice || 0;
            const currentTotalPrice = getTotalPrice();
        ...
        if (condType === 'product') {
            ...
            if (bumpConfig.conditions.requiredProduct) {
                const productData = getProductDataById(bumpConfig.conditions.requiredProduct.id);
                productName = productData?.name || 'produk';
                productQuantity = bumpConfig.conditions.requiredProduct.minQuantity || 1;
                currentProductQuantity = getProductQuantity(bumpConfig.conditions.requiredProduct.id);
            }
            
            message = message
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{productQuantity}/g, `<strong>${productQuantity}</strong>`)
                .replace(/{currentProductQuantity}/g, `<strong>${currentProductQuantity}</strong>`);
        }
        
        // For 'price-product' type
        if (condType === 'price-product') {
            let productName = 'produk';
            let productPrice = 0;
            ...
            if (bumpConfig.conditions.requiredProduct) {
                const productData = getProductDataById(bumpConfig.conditions.requiredProduct.id);
                productName = productData?.name || 'produk';
                productPrice = bumpConfig.conditions.requiredProduct.minPrice || 0;
                currentProductPrice = getProductPrice(bumpConfig.conditions.requiredProduct.id);
            }
            
            message = message
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{productPrice}/g, `<strong>${formatPrice(productPrice)}</strong>`)
                .replace(/{currentProductPrice}/g, `<strong>${formatPrice(currentProductPrice)}</strong>`);
        }
    }
...
function updateBumpNotification(bumpIndex, conditionStatus) {
    ...
    const isBumpSelected = checkbox?.checked || false;

    // ✅ PATCH: If bump is selected, always hide notification
    if (isBumpSelected) {
    ...
    // ✅ PATCH: If conditions are met - CREATE OR UPDATE success notification
    if (conditionStatus.met) {
        // Show success message if enabled
        if (window.cartConfig.notifications.showSuccessMessage) {
            const message = getSuccessMessage(
                bumpConfig.conditions.type, 
                bumpConfig.productName, 
                bumpConfig, 
                conditionStatus
            );
                ...
                if (bumpContainer) {
                    ...
                    if (targetElement) {
                        targetElement.insertAdjacentElement(
                            insertMethod === 'beforebegin' ? 'beforebegin' : 'afterend', 
                            notification
                        );
                    }
                }
            ...
            // ✅ Remove notification if showSuccessMessage is disabled
            if (notification && notification.parentNode) {
                notification.remove();
            }
        }
    ...
    // ✅ EXISTING: If conditions are not met and user has started ordering
    if (!conditionStatus.met && conditionStatus.hasStarted) {
        ...
        // ✅ UPDATE notification to pending state
        ...
        // ✅ Remove notification if conditions not started
        if (notification && notification.parentNode) {
            notification.remove();
        }
    }
...
function getProductDataById(productId) {
    ...
    // Ambil nama produk
    let productName = '';
    const productNameDiv = productNameCell?.querySelector('.product-name');
    if (productNameDiv) {
        productName = productNameDiv.textContent.trim();
    } else {
        productName = productNameCell ? productNameCell.textContent.trim() : '';
    }
    
    // Ambil image URL (jika ada)
    let productImage = null;
    const imgElement = productNameCell?.querySelector('img.product-picture-thumb');
    if (imgElement) {
        productImage = imgElement.src || imgElement.getAttribute('data-src');
    }
    
    if (window.cartConfig.debugMode) {
        console.log(`📦 Product ${productId} found:`, productName, productImage ? '🖼️' : '');
    }
...
function showProductAsOrderBump(productId, quantity) {
    ...
    if (productRow) {
        ...
        if (productNameCell && !productNameCell.querySelector('.order-bump-badge-table')) {
            ...
            const productDesc = productNameCell.querySelector('.product-description');
            
            if (productDesc) {
                // Insert BEFORE product-description
                productNameCell.insertBefore(badge, productDesc);
            } else {
                // No product-description, append at the end
        ...
        const subtotalCell = productRow.querySelector('.subtotal-price');
        if (subtotalCell) {
            updateOriginalPriceDisplay(productId, subtotalCell);
        }
...
function init() {
    ...
    document.querySelectorAll('input[type="radio"]').forEach(radio => {
        ...
        customDiv.className = 'radio-custom';
        radio.insertAdjacentElement('afterend', customDiv);
    });

    document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
        const customCheckDiv = document.createElement('div');
        customCheckDiv.className = 'checkbox-custom';
        checkbox.insertAdjacentElement('afterend', customCheckDiv);
    });
}

// ========== ORIGINAL PRICE DISPLAY FUNCTIONS ==========

/**
 * Check if product should show original price
 * @param {number} productId - Product ID to check
 * @returns {boolean} - True if should show original price
 */
function shouldShowOriginalPrice(productId) {
    const config = window.cartConfig.originalPrice;
    
    if (!config || !config.enabled) {
        return false;
    }
    
    if (config.applyTo === 'all-products') {
        return true;
    }
    
    if (config.applyTo === 'bump-only') {
        // Check if product is used in any enabled order bump
        return window.cartConfig.orderBumps.some(bump => 
            bump.enabled && bump.productId === productId && bump.originalPrice
        );
    }
 ...
 */
function getOriginalPriceForProduct(productId) {
    // For bump-only mode, get from orderBumps config
    if (window.cartConfig.originalPrice?.applyTo === 'bump-only') {
        const bump = window.cartConfig.orderBumps.find(b => 
            b.enabled && b.productId === productId && b.originalPrice
        );
        return bump ? bump.originalPrice : null;
    }
    
    // For all-products mode, you might want to store originalPrice in a separate config
    // or calculate it from product data
    return null;
}

/**
 * Update original price display in subtotal cell
 * @param {number} productId - Product ID
 * @param {HTMLElement} subtotalCell - Subtotal price cell element
 */
function updateOriginalPriceDisplay(productId, subtotalCell) {
    if (!subtotalCell) return;
    
    // 🔥 CRITICAL: Prevent infinite loop
    if (subtotalCell.dataset.updatingPrice === 'true') {
        return;
    }
    
    const currentSubtotal = parseFloat(subtotalCell.textContent.trim()) || 0;
    const shouldShow = shouldShowOriginalPrice(productId);
    
    // Remove existing original price display
    const existingDisplay = subtotalCell.querySelector('.original-price-display');
    if (existingDisplay) {
        existingDisplay.remove();
    }
    
    // Only show if subtotal > 0 and should show
    if (shouldShow && currentSubtotal > 0) {
        const originalPrice = getOriginalPriceForProduct(productId);
        
        if (originalPrice && originalPrice > currentSubtotal) {
            const quantity = getProductQuantity(productId);
            const totalOriginalPrice = originalPrice * quantity;
            
            // 🔥 Set flag BEFORE updating DOM
            subtotalCell.dataset.updatingPrice = 'true';
            
            // Create and insert original price display (hardcoded format)
            const formattedPrice = totalOriginalPrice.toFixed(2);
            const originalPriceHTML = `<span class="original-price-display">${formattedPrice}</span>`;
            subtotalCell.insertAdjacentHTML('afterbegin', originalPriceHTML);
            
            // 🔥 Clear flag AFTER updating DOM
            setTimeout(() => {
                delete subtotalCell.dataset.updatingPrice;
            }, 50);
            
            if (window.cartConfig.debugMode) {
                console.log(`💰 Original price shown for product ${productId}:`, totalOriginalPrice);
            }
        }
    }
}
...
/**
 * Setup monitoring for all products' subtotal changes
 */
function setupOriginalPriceMonitoring() {
    if (!window.cartConfig.originalPrice?.enabled) {
        return;
    }
    
    const productRows = document.querySelectorAll('.product[data-id]');
    
    productRows.forEach(row => {
        const productId = parseInt(row.getAttribute('data-id'));
        const subtotalCell = row.querySelector('.subtotal-price');
        ...
        // Setup MutationObserver untuk monitor changes
        const observer = new MutationObserver(() => {
            updateOriginalPriceDisplay(productId, subtotalCell);
        });
        
        observer.observe(subtotalCell, {
            childList: true,
            characterData: true,
            subtree: true
        });
        ...
        if (window.cartConfig.debugMode) {
            console.log(`👀 Original price monitoring setup for product ${productId}`);
        }
    });
}

/**
 * Refresh all original price displays
 */
function refreshAllOriginalPrices() {
    const productRows = document.querySelectorAll('.product[data-id]');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (targetElement) {
        window.cartConfig.orderBumps.forEach((bumpConfig, index) => {
            ...
            // 👇 NEW: Ambil product data untuk image
            let productImageHTML = '';
            if (processedConfig.productId) {
                const productData = getProductDataById(processedConfig.productId);
