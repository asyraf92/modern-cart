...
function checkSingleCondition(condition) {
    ...
    if (condType === 'price-total' || condType === 'price-both') {
    ...
    if (condType === 'price-product' || condType === 'price-both') {
        // Multiple products support
        if (condition.requiredProducts && Array.isArray(condition.requiredProducts)) {
            const productResults = condition.requiredProducts.map(reqProduct => {
    ...
    if (condType === 'hybrid') {
        const currentTotal = getTotalQuantity();
        const requiredTotal = condition.minTotalQuantity || 0;
        
        const currentTotalPrice = getTotalPrice();
        const requiredTotalPrice = condition.minTotalPrice || 0;
        
        if (currentTotal > 0 || currentTotalPrice > 0) {
            result.hasStarted = true;
        }
...
/**
 * Get formatted product names string from product IDs
 * @param {Array} productIds - Array of product IDs
 * @returns {string} - Formatted product names
 */
...
/**
 * Generate list-based message for 'both' type with multiple products
 * Shows total quantity requirement + individual product requirements
 * @param {string} combineLogic - 'all' or 'any'
 * @param {Object} bumpConfig - Bump configuration
 * @param {Object} conditionStatus - Full condition status with totals
 * @returns {string} - Formatted HTML message with list
 */
function getMultipleProductsBothMessage(combineLogic, bumpConfig, conditionStatus) {
    const templates = window.cartConfig.notifications?.messages?.multipleProducts?.[combineLogic];
    ...
    // Get 'both' heading template
    let heading = templates.bothHeading || 
        'Sila lengkapkan jumlah kuantiti {totalQuantity} unit - baki lagi {remainingTotal} unit dan penuhi syarat produk berikut untuk unlock {bumpName}:';
    
    // Get total quantity info from condition
    let totalQuantity = 0;
    let currentTotal = getTotalQuantity();
    
    // Find the condition to get minTotalQuantity
    if (bumpConfig.conditions.multipleConditions) {
        const matchingCondition = bumpConfig.conditions.multipleConditions.find(c => 
            c.type === 'both' && c.requiredProducts && c.requiredProducts.length > 1
        );
        if (matchingCondition) {
            totalQuantity = matchingCondition.minTotalQuantity || 0;
        }
    } else if (bumpConfig.conditions.type === 'both') {
        totalQuantity = bumpConfig.conditions.minTotalQuantity || 0;
    }
    ...
    heading = heading
        ...
        .replace(/{remainingTotal}/g, `<strong>${conditionStatus.remainingTotal || 0}</strong>`);
    
    // Get item template
    const itemTemplate = templates.bothItemTemplate || templates.itemTemplate ||
        'Perlukan {productQuantity} unit {productName}. Sekarang ada {currentProductQuantity} unit dalam cart, perlu tambah lagi {remainingProductQuantity} unit.';
    
    // Build list items from productDetails
    const listItems = conditionStatus.productDetails.map((detail, index) => {
        // Get product name
        const productData = getProductDataById(detail.id);
        const productName = productData?.name || `Produk ${detail.id}`;
...
/**
 * Generate list-based message for 'price-both' type with multiple products
 * Shows total price requirement + individual product price requirements
 * @param {string} combineLogic - 'all' or 'any'
 * @param {Object} bumpConfig - Bump configuration
 * @param {Object} conditionStatus - Full condition status with totals
 * @returns {string} - Formatted HTML message with list
 */
function getMultipleProductsPriceBothMessage(combineLogic, bumpConfig, conditionStatus) {
    const templates = window.cartConfig.notifications?.messages?.multipleProducts?.[combineLogic];
    ...
    // Get 'price-both' heading template
    let heading = templates.priceBothHeading || 
        'Sila lengkapkan jumlah harga {totalPrice} - baki lagi {remainingTotalPrice} dan penuhi syarat harga produk berikut untuk unlock {bumpName}:';
    
    // Get total price info from condition
    let totalPrice = 0;
    let currentTotalPrice = getTotalPrice();
    
    // Find the condition to get minTotalPrice
    if (bumpConfig.conditions.multipleConditions) {
        const matchingCondition = bumpConfig.conditions.multipleConditions.find(c => 
            c.type === 'price-both' && c.requiredProducts && c.requiredProducts.length > 1
        );
        if (matchingCondition) {
            totalPrice = matchingCondition.minTotalPrice || 0;
        }
    } else if (bumpConfig.conditions.type === 'price-both') {
        totalPrice = bumpConfig.conditions.minTotalPrice || 0;
    }
    ...
    heading = heading
        ...
        .replace(/{remainingTotalPrice}/g, `<strong>${formatPrice(conditionStatus.remainingTotalPrice || 0)}</strong>`);
    
    // Get item template
    const itemTemplate = templates.priceBothItemTemplate || templates.priceItemTemplate ||
...
function getPendingMessage(condType, bumpConfig, conditionStatus) {
    ...
    if (bumpConfig.conditions.multipleConditions) {
        ...
        if (customTemplate) {
            if (combineLogic === 'all') {
                ...
                if (closestCondition) {
                    const conditionText = getConditionMessage(closestCondition.type, closestCondition.details, bumpConfig, conditionStatus);
                    
                    // ✅ FIX: Check if user wants to use custom template
                    // If list-based message AND user has custom template, wrap it
                    if (conditionText.includes('<div class="multi-product-notification">')) {
                        // Check if template has {conditions} placeholder
                        if (customTemplate.includes('{conditions}')) {
                            // Replace {conditions} with the list-based message
                            return customTemplate
                                .replace(/{conditions}/g, conditionText)
                                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
                        } else {
                            // No {conditions} placeholder, return list message as-is
                            return conditionText;
                        }
                    }
                    
                    // Regular text-based condition message
                    return customTemplate
    ...
    const fallbackMessages = {
        'total': 'Tambah {remaining} item lagi untuk unlock {bumpName}',
        'product': 'Tambah {remaining} {productName} lagi untuk unlock {bumpName}',
        'both': 'Sila lengkapkan jumlah kuantiti {totalQuantity} unit - baki lagi {remainingTotal} unit mana-mana produk - termasuk penambahan lagi {remainingProduct} unit {productName} untuk unlock {bumpName} pada harga diskaun.',
        'price-total': 'Tambah lagi {remainingPrice} untuk unlock {bumpName}',
        'price-product': 'Tambah lagi {remainingPrice} untuk {productName} untuk unlock {bumpName}',
        'price-both': 'Sila lengkapkan jumlah harga {totalPrice} - baki lagi {remainingTotalPrice} mana-mana produk - termasuk penambahan lagi {remainingProductPrice} untuk {productName} untuk unlock {bumpName} pada harga diskaun.',
        'hybrid': 'Tambah {remainingQuantity} item & {remainingPrice} lagi untuk unlock {bumpName}'
    };
    ...
    const typesWithMultipleProducts = ['product', 'both', 'price-product', 'price-both'];
    ...
    if (condType === 'both' || condType === 'price-both') {
        return getConditionMessage(condType, conditionStatus, bumpConfig, conditionStatus);
    }
    ...
    switch(condType) {
        ...
        case 'both': {
            ...
            if (details.productDetails && details.productDetails.length > 1) {
                ...
                // Use fullConditionStatus if provided, otherwise use details
                const statusToUse = fullConditionStatus || details;
                
                // Generate 'both' message with total + product list
                return getMultipleProductsBothMessage(combineLogic, bumpConfig, statusToUse);
            }
            
            // Single product - use template from config
            const templates = window.cartConfig.notifications?.messages?.both;
            ...
            if (bumpConfig.conditions.multipleConditions) {
                const matchingCondition = bumpConfig.conditions.multipleConditions.find(c => {
                    if (c.type === 'both' && c.requiredProduct) {
                        return details.productDetails?.some(pd => pd.id === c.requiredProduct.id);
                    }
                    return false;
                });
            ...
            return template
                ...
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{totalQuantity}/g, `<strong>${totalQuantity}</strong>`)
                .replace(/{remainingTotal}/g, `<strong>${details.remainingTotal || 0}</strong>`)
                .replace(/{remainingProduct}/g, `<strong>${details.remainingProduct || 0}</strong>`);
        }
        ...
        case 'price-both': {
            // ✅ NEW: Enhanced 'price-both' handling
            ...
            if (details.productDetails && details.productDetails.length > 1) {
                ...
                // Generate 'price-both' message with total price + product list
                return getMultipleProductsPriceBothMessage(combineLogic, bumpConfig, statusToUse);
            }
            
            // Single product - use template from config
            const templates = window.cartConfig.notifications?.messages?.['price-both'];
            let template = templates?.pending || 
                'Sila lengkapkan jumlah harga {totalPrice} - baki lagi {remainingTotalPrice} mana-mana produk - termasuk penambahan lagi {remainingProductPrice} untuk {productName} untuk unlock {bumpName} pada harga diskaun.';
            ...
            if (bumpConfig.conditions.multipleConditions) {
                const matchingCondition = bumpConfig.conditions.multipleConditions.find(c => {
                    if (c.type === 'price-both' && c.requiredProduct) {
                        return details.productDetails?.some(pd => pd.id === c.requiredProduct.id);
                    }
                    return false;
                });
            ...
            return template
                ...
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{totalPrice}/g, `<strong>${formatPrice(totalPrice)}</strong>`)
                .replace(/{remainingTotalPrice}/g, `<strong>${formatPrice(details.remainingTotalPrice || 0)}</strong>`)
                .replace(/{remainingProductPrice}/g, `<strong>${formatPrice(details.remainingProductPrice || 0)}</strong>`);
        }
        
        case 'hybrid': {
            const parts = [];
            if (details.remainingTotal > 0) {
                parts.push(`${details.remainingTotal} item`);
            }
            if (details.remainingTotalPrice > 0) {
                parts.push(`${formatPrice(details.remainingTotalPrice)}`);
            }
            
            return parts.length > 0 ? parts.join(' & ') + ' lagi' : '';
        }
...
function replacePlaceholdersInMessage(template, bumpConfig, conditionStatus) {
    ...
    message = message.replace(/{remaining}/g, `<strong>${conditionStatus.remainingTotal || conditionStatus.remainingProduct || 0}</strong>`);
    ...
    message = message.replace(/{remainingQuantity}/g, `<strong>${conditionStatus.remainingTotal || 0}</strong>`);
    message = message.replace(/{remainingPrice}/g, `<strong>${formatPrice(conditionStatus.remainingTotalPrice || conditionStatus.remainingProductPrice || 0)}</strong>`);
...
function updateBumpNotification(bumpIndex, conditionStatus) {
    ...
    if (!conditionStatus.met && conditionStatus.hasStarted) {
        // ✅ Use updated getPendingMessage with user-defined template
...
function updateBumpVisibility(bumpIndex) {
    ...
    if (hasStarted || shouldShow) {
        ...
        if (!shouldShow) {
            ...
            if (window.cartConfig.debugMode) {
                console.log(`ðŸ"' Order bump ${bumpIndex} shown but DISABLED (conditions not met)`);
            }
        } else {
