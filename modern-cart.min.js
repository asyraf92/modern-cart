...
function getConditionMessage(condType, details, bumpConfig, fullConditionStatus = null, originalCondition = null) {
    switch(condType) {
        ...
        case 'product': {
            ...
            // ✅ Single product - wrap dalam div untuk new line
            ...
            const defaultMessages = window.cartConfig.notifications?.messages || {};
            const template = defaultMessages['product']?.pending || 'Tambah {remaining} {productName} lagi untuk unlock {bumpName}';
            
            const message = template
                .replace(/{remaining}/g, `<strong>${details.remainingProduct}</strong>`)
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
            
            // ✅ Wrap dalam div seperti multi-product-notification untuk consistency
        ...
        case 'price-product': {
            ...
            const defaultMessages = window.cartConfig.notifications?.messages || {};
            const template = defaultMessages['price-product']?.pending || 'Tambah lagi {remainingPrice} untuk {productName} untuk unlock {bumpName}';
            
            const message = template
                .replace(/{remainingPrice}/g, `<strong>${formatPrice(details.remainingProductPrice)}</strong>`)
                .replace(/{productName}/g, `<strong>${productName}</strong>`)
                .replace(/{bumpName}/g, `<strong>${bumpConfig.productName}</strong>`);
