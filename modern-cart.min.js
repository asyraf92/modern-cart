...
function getPendingMessage(condType, bumpConfig, conditionStatus) {
    ...
    if (bumpConfig.conditions.multipleConditions) {
        ...
        if (customTemplate) {
            ...
            let conditionsText = '';
            const separator = separators[combineLogic] || '&';
            
            // Build condition text with appropriate separator
            if (wrappedConditions.length === 1) {
                conditionsText = wrappedConditions[0];
            } else if (wrappedConditions.length === 2) {
                conditionsText = wrappedConditions.join(` ${separator} `);
            } else {
                conditionsText = wrappedConditions.slice(0, -1).join(', ') + ` ${separator} ` + wrappedConditions[wrappedConditions.length - 1];
            }
